...
int main(int argc, char **argv)
{
	...
	{
	 	printf("Error, data to send to DO_B out of range\n");
		printf("Usage %s <data>\n", argv[0]);
		printf("\t\tformat for data is hex: 0x?? (you have to supply the 0x prefix!)\n");
		exit(1);
	}
	devaddr = 0x00;
	printf("sending 0x%X to Digital Out Type B device 0x%X\n", data, devaddr);
	b = 0x08;   // DO_B send
	serialWrite(fd, b);
	b = 0x00;  // DO_B address (zero for now)
	serialWrite(fd, b);
	b = data;
	printf("sending %i DO_B ", b);
	serialWrite(fd, b);
